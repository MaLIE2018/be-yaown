...
bankRouter.post('/auth/bunq/code', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    //create all accounts
    const accounts = await BUNQInstance.get(`/v1/${session.Response[2].UserPerson.id}/monetary-account-bank`, {headers:{'X-Bunq-Client-Authentication': session.Response[1].Token.token}}).then;
