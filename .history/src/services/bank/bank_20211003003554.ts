...
//get balances
bankRouter.post('/balances', async (req: Request, res: Response, next: NextFunction) => {
  try {
    let balancesSets = []
    let bunqBalancesSets = []
    const lastRefresh = toDate(req.user.lastBalRefresh);
    if (differenceInHours(new Date(), lastRefresh) < 6) {
      res.status(429).send({ m: differenceInHours(new Date(), lastRefresh) });
    }
    const requisitions = req.user.agreements.reduce((acc: { id: string; accountId: string }[], curr: Agreement) => {
    ...
    balances.forEach(async balance => await Models.Accounts.findByIdAndUpdate(balance.id, {$set: {balances: [balance.bs]}})
    );
    //only four times a day
