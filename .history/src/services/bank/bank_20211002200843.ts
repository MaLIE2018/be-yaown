...
import { MonetaryAccount } from 'types/bunq';
...
bankRouter.post('/auth/bunq/code', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const accountMap: { id: string; accountId: string }[] = [];
    //create all accounts
    const accounts = await BUNQInstance.get(`/v1/${session.Response[2].UserPerson.id}/monetary-account-bank`, {
    ...
    if (accounts) {
      accounts.forEach(async (account:MonetaryAccount) => {
        const newAccountObj = {
          iban: account.MonetaryAccountBank.alias.find((item) => item.type === 'IBAN').value,
          name: account.MonetaryAccountBank.description,
          currency: account.MonetaryAccountBank.description,
          userId: req.user._id,
          accountId: account.MonetaryAccountBank.id,
          balances: [
            {
              balanceAmount: {
          ...
          aspspId: 'BUNQ_NE',
          bankName: 'Bunq Bank of the Free',
