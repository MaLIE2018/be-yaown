      ...
      accounts.forEach(async (account:MonetaryAccount) => {
        const newAccountObj = {
          ...
          balances: [
            {
              balanceAmount: {
                amount: Number(account.MonetaryAccount.balance.value),
                currency: account.MonetaryAccount.balance.currency,
              },
        ...
        //create new Account
        const newAccount = new Models.Accounts(newAccountObj);
        accountMap.push({ id: newAccount._id, accountId:account.MonetaryAccountBank.id })
        await newAccount.save();
      });
    }
    //agreement
