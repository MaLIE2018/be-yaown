  ...
  try {
    const auth = await axios
      ...
      if (auth.status === 200) {
        ...
        const session = await BUNQInstance.post('/v1/session-server', body ,{headers:{'X-Bunq-Client-Authentication': process.env.BUNQ_INSTALLATION_TOKEN, 'X-Bunq-Client-Signature':signature}}).then(session => session.data)
