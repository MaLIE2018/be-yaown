...
bankRouter.post('/auth/bunq/code', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const accountMap: { id: string; accountId: string }[] = [];
    //create all accounts
    const accounts = await BUNQInstance.get(`/v1/user/${session.Response[2].UserPerson.id}/monetary-account-bank`, {
    ...
    if (accounts) {
      accounts.forEach(async (account:MonetaryAccount) => {
        if(account.MonetaryAccountBank.status === "ACTIVE" && account.MonetaryAccountBank.user_id === session.Response[2].UserPerson.id){
          const newAccountObj:Account = {
            iban: account.MonetaryAccountBank.alias.find((item:Alias) => item.type === 'IBAN').value,
            name: account.MonetaryAccountBank.description,
            currency: account.MonetaryAccountBank.description,
            userId: req.user._id,
            accountId: account.MonetaryAccountBank.id.toString(),
            savingRate: 0,
            ...
            balances: [
              {
                balanceAmount: {
                  amount: Number(account.MonetaryAccountBank.balance.value),
                  currency: account.MonetaryAccountBank.balance.currency,
                },
          ...
          //create new Account
          const newAccount = new Models.Accounts(newAccountObj);
          accountMap.push({ id: newAccount._id, accountId:account.MonetaryAccountBank.id.toString() })
          await newAccount.save();
        }
      });
    }
