...
bankRouter.get('/balances', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const bunqRequisitions = req.user.agreements.find((agreement: Agreement) => agreement.aspsp_id === 'BUNQ_NE');
    if (requisitions.length === 0) next(createError(404, { m: 'No available accounts' }));
    // get Balances over api
    const balancesSets = await Promise.all(
    ...
    const bunqBalancesSets = await Promise.all(
      bunqRequisitions.map(async (accountMap: { id: string; accountId: string }) => {
          return await BUNQInstance.get(`/v1/user/${ff}/monetary-account/${accountMap.id}`, {
            headers: { 'X-Bunq-Client-Authentication': process.env.BUNQ_INSTALLATION_TOKEN },
          }).then()
        }
      })
    );
