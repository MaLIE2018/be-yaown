      ...
      requisitions.map(
        async (accountMap: { id: string; accountId: string }) =>{
          if(uuidValidate(accountMap.id)){
            ...
            return await BUNQInstance.get(`/v1/user/${ff}/monetary-account/${accountMap.id}`,{headers:{'X-Bunq-Client-Authentication': process.env.BUNQ_INSTALLATION_TOKEN}}).then
          }
        }
      )
    );  
