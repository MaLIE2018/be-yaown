      ...
      bunqBalancesSets = await Promise.all(
        bunqAgreement.accounts.map(async (accountMap: { id: string; accountId: string }) => {
          return await BUNQInstance.get(
            ...
            let key: string = '';
            if (account.data.Response?.MonetaryAccountBank) {
              key = 'MonetaryAccountBank';
            } else if (account.data.Response?.MonetaryAccountJoint) {
              key = 'MonetaryAccountJoint';
            } else if (account.data.Response?.MonetaryAccountSavings) {
...
console.log(account.data.Response);
            return {
              ...
              bs: {
                balanceAmount: {
                  amount: Number(account.data.Response[key].balance.value),
                  currency: account.data.Response[key].balance.currency,
                },
    ...
    balances.forEach(
      async (balance) => await Models.Accounts.findByIdAndUpdate(balance.id, { $set: { balances: [balance.bs] } })
    );
    //only four times a day
